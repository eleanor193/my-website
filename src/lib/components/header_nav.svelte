    ...
    {#each nav as { children, link, text }}
      ...
      {:else if children}
        <li class="dropdown dropdown-hover">
          <div tabindex="0" role="button" class='!rounded-btn gap-1' class:font-bold={children.some(({ link }) => link === path)}>
            {text}
            <span class='i-heroicons-solid-chevron-down -mr-1' />
          </div>
          <!-- svelte-ignore a11y-no-noninteractive-tabindex -->
          <ul class='dropdown-content menu rounded-box bg-base-100 text-base-content shadow-lg p-2 w-32' tabindex='0'>
            {#each children as { link, text }}
              <li>
                <a class:font-bold={link === path} href={link}>{text}</a>
              </li>
